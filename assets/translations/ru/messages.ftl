...
msg-user-confirm-delete =
    <b>⚠️ Подтверждение удаления</b>

    Вы действительно хотите **удалить** пользователя <b>{ $name }</b> (ID: <code>{ $telegram_id }</code>)?

    <i>Пользователь и его подписка будут полностью удалены из базы данных и из панели Remnawave. Это действие необратимо!</i>

ntf-user-deleted-success = Пользователь успешно удален
